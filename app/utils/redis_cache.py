    ...
    if _redis_client is None:
        try:
            ...
            if "localhost" in redis_url or "127.0.0.1" in redis_url:
                try:
                    # Если 'redis' резолвится, значит мы внутри Docker, заменяем localhost на redis
                    socket.gethostbyname('redis')
                    redis_url = redis_url.replace("localhost", "redis").replace("127.0.0.1", "redis")
                    logger.debug(f"[REDIS] Заменен localhost на имя сервиса для Docker (socket resolved): {redis_url}")
                except socket.error:
                    pass # Локальная разработка, оставляем localhost
            elif "://redis:" in redis_url or "://art_generation_redis_local:" in redis_url:
                try:
                    socket.gethostbyname('redis')
                except socket.error:
                    # Если 'redis' не резолвится, но запрошен, значит мы локально - заменяем на localhost
                    redis_url = redis_url.replace("://redis:", "://localhost:").replace("://art_generation_redis_local:", "://localhost:")
                    logger.debug(f"[REDIS] Заменено имя сервиса на localhost для локальной разработки (fallback): {redis_url}")
